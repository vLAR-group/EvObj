...
def main(cfg, logger):
    ...
    print(f"Evo data collection and training will occur every {cfg.evo_T} epochs at: {evo_epochs}")
    ...
    #########################
    trainer = training_scannet_ddpm.Trainer(
        mask3d, objnet, actor, critic, logger, train_dataset, val_dataset,
        cfg.save_path, cfg, use_norm=cfg.use_norm, use_label=False, discern_voxel_size=cfg.voxel_size,
    )
