...
def save_ply(pc_tensor: torch.Tensor, filepath: str):
    import open3d as o3d
    pts = pc_tensor.detach().cpu().numpy()
...
def linear_beta_schedule(timesteps):
    ...
    scale = 1000 / timesteps  # scale=1 may suffice; default DDPM uses beta in [1e-4, 0.02]
...
class EvoDatasetFromFiles(Dataset):
    ...
    def __init__(self, data_dir, num_point=4096, voxel_size=0.05, cfg=None):
        ...
        # Find all PLY files with mask information in the directory
        self.ply_files = self._find_ply_files()
        print(f"Found {len(self.ply_files)} evo data files in {data_dir}")
    
    def _find_ply_files(self):
        """Find all PLY files with mask information in the data directory"""
        ply_files = []
    ...
    def __getitem__(self, idx):
        ...
        # Load PLY file with mask information
        pcd = o3d.io.read_point_cloud(ply_file)
        points = np.asarray(pcd.points)  # [N, 3]
        
        # Read mask directly from PLY file
    ...
    def _read_mask_from_ply(self, ply_file):
        """Read mask information from PLY file"""
        masks = []
        with open(ply_file, 'r') as f:
            # Skip header until we reach the data
...
class Trainer(object):
    def __init__(self, model, objnet, PPO_actor, PPO_critic, logger, train_dataset, val_dataset, save_path, cfg=None, use_norm=True, use_label=False, discern_voxel_size=0.05):
        ...
        print(f"Trainer comp_backend={self.comp_backend}")
        ...
        print(f"Trainer discern_backend={self.discern_backend}")
        ...
        # Evolution settings 
        self.evo_batch_size = 32  # Run discern evolution when enough evo samples are collected
        self.evo_epochs_per_batch = 40  # Train each batch multiple times
        self.evo_thr = self.cfg.cd_thr
        self.evo_lr = 1e-3
        self.evo_sample_counter = 0  # Counter for naming saved evo PLY files
        self.evo_epochs = self.cfg.evo_epochs  # Epochs to collect and train evo data

        print(f'setting evo_thr: {self.evo_thr}')
        print(f'setting evo_epochs: {self.evo_epochs}')
        ...
        for iter in range(self.batch_iter):
            ...
            if self.training_iter % self.logging_interval == 0:
                ...
                self.logger.info(
                    ...
                    'dice: {:.3f}, class: {:.3f}, evo_discern: {:.3f}, lr: {:.3e}, Traj: {:.2f}, data time: {:.1f}s, optimize time: {:.1f}s, Elapsed time: {:.1f}s ({} iters)'.format(
    ...
    def center_points(self, points):
        """Center point cloud at origin (translation only)."""
        centroid = points.mean(dim=0)
        return points - centroid

    def prepare_point_transformer_input(self, points, masks=None, device='cuda'):
        """Batch Point Transformer input: coord / feat / offset (+ optional labels)."""
    ...
    def compute_convergence(self, mask_list, pc_list, prob_list):
        ...
        if np.array(valid_mask).sum()>0:
            ...
            # Complete point clouds with the completion network first
            comp_centers = comp_input.mean(dim=1, keepdim=True)  # [B, 1, 3]
            comp_input = comp_input - comp_centers  # [B, N, 3]
            dists = torch.norm(comp_input, dim=2)  # [B, N]
            comp_scale = dists.max(dim=1, keepdim=True).values  # [B, 1]
            # Avoid division by zero
            comp_scale = comp_scale + 1e-6
            # Normalize
            comp_input = comp_input / comp_scale.unsqueeze(-1)  # [B, N, 3]
            with torch.no_grad():
                out = self.comp_net(comp_input.cuda())
            comp_output = out[-1]  # [B, 2048, 3]
            comp_output = comp_output * comp_scale.unsqueeze(-1) + comp_centers  # [B, M, 3] denormalize

            # If output point count is not 1024, resample to 1024
            B, N, _ = comp_output.shape
            if N != 1024:
                # Random resample to 1024 points
                if N > 1024:
            ...
            # encoder_input_pc and query_pc both use the same completed point cloud
            encoder_input_pc = comp_output
            query_pc = comp_output

            # Batched: VAE pose, cond net, DDIM latent, SDF decode, CD to mesh samples
            head = 0
            with torch.no_grad():
    ...
    def voxelize_batch(self, points, masks, voxel_size=0.05, device='cuda'):
        ...
        for b in range(batch_size):
            ...
            grid_coords = np.floor(grid_coords * scale)
            # Unique voxelization
            grid_coords_unique, unq_idx, unq_inv = np.unique(grid_coords, return_index=True, return_inverse=True, axis=0)
            # Convert to tensors
            grid_coords_unique = torch.from_numpy(grid_coords_unique).long().to(device)
            unq_idx = torch.from_numpy(unq_idx).long()
            unq_inv = torch.from_numpy(unq_inv).long().to(device)
            ...
            batch_offsets.append(batch_offsets[-1] + grid_coords_unique.shape[0])
    ...
    def compute_reward_CD(self, idx, valid_mask, pc2meshs, mesh2pcs, traj_id=None, sdfmask=None):
        if valid_mask[idx]:
            ...
            # Collect data for discerning evolution if CD is below evo_thr and current epoch is in evo_epochs list
    ...
    def evolve_discern_network(self):
        """Evolve discern_net on collected evo PLY data (same structure as training_scannet_vae)."""
    ...
    def train_model(self, epochs):
        ...
        for epoch in range(start, epochs):
            for batch_idx, batch in enumerate(train_data_loader):
                # print(f"Training batch {batch_idx+1}/{len(train_data_loader)}")
                self.train_batch(batch, batch_idx+1, epoch, len(train_data_loader))
            if epoch in self.evo_epochs:
                evo_data_dir = os.path.join(self.save_path, 'evo_data', f'epoch_{epoch:03d}')
    ...
    def save_checkpoint(self, epoch):
        ...
        if not os.path.exists(path):
            torch.save({'epoch':epoch,
                ...
                'discern_net_state_dict': self.discern_net.state_dict(),
                'opt_discern_state_dict': self.optimizer_discern.state_dict(),
                }, path)
        ...
        for batch_idx, batch in enumerate(val_data_loader):
            ...
            if vis:
                with torch.no_grad():
                    ...
                    write_ply(os.path.join(self.cfg.save_path + '/vis_scene', scene_name[0] + 'preds_ddpm.ply'), [full_pc, predcolor.astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])
                    ...
                    write_ply(os.path.join(self.cfg.save_path + '/vis_scene', scene_name[0] + 'gt_ddpm.ply'), [full_pc, gtcolor.astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])
    ...
    def extract_shape_pts(self, decoder, embedding, N=32, sample_pts_num=10000):
        ...
        voxel_size = 2.0 / N  # grid spacing over [-1,1]^3 (N voxels per axis)
    ...
    def generate_mesh(self, decoder, embedding, N=32, path=None):
        voxel_origin = [-1.0, -1.0, -1.0]
        voxel_size = 2.0 / N  # grid spacing over [-1,1]^3
        overall_index = torch.arange(0, N ** 3, 1, out=torch.LongTensor())
