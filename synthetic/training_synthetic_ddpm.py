...
class Trainer(object):
    def __init__(self, model, objnet, PPO_actor, PPO_critic, logger, train_dataset, val_dataset, val_RL_dataset, save_path, cfg=None, use_norm=True, use_label=False, cd_thr=0.10):
    ...
    def refresh_info(self):
        ...
        self.loss_dict = {'loss': 0, 'ppo loss': 0 , 'actor loss': 0, 'critic loss': 0, 'ent loss': 0,
                          'discern loss': 0, 'mask loss': 0, 'dice loss': 0, 'class loss': 0}
    ...
    def train_batch(self, batch, batch_idx, epoch, loader_size):
        ...
        for iter in range(self.batch_iter):
            ...
            if len(valid_bs) > 0:  ### means in some scene, we found object in RL collecting data process
                mask_loss, dice_loss, class_loss = self.compute_discern_loss(output_train, output_train["pred_masks"], pseudo_mask_list, valid_bs)
                discern_loss = 2 * class_loss + 5 * mask_loss + 2 * dice_loss

            else:
                mask_loss = dice_loss = class_loss = torch.tensor(0)
                discern_loss = 2 * class_loss + 5 * mask_loss + 2 * dice_loss

            loss = ppo_loss + discern_loss
            # loss = discern_loss
            loss.backward()
            ...
            self.loss_dict['discern loss'] += discern_loss.item()
            ...
            if self.training_iter % self.logging_interval == 0:
                ...
                self.logger.info(
                    '{} Epoch: {} [{}/{} ({:.0f}%)]{}, Loss: {:.3f}, ppo: {:.3f}, actor: {:.3f}, critic: {:.3f}, ent: {:.3f}, StepR: {:.2f}, discern: {:.3f}, mask: {:.3f}, '
                    'dice: {:.3f}, class: {:.3f}, lr: {:.3e}, Traj: {:.2f}, data time: {:.1f}s, optimize time: {:.1f}s, Elapsed time: {:.1f}s ({} iters)'.format(
                        ...
                        self.loss_dict['discern loss'], self.loss_dict['mask loss'], self.loss_dict['dice loss'],
    ...
    def compute_discern_loss(self, output, score, pseudo_mask_list, valid_bs):
    ...
    def compute_convergence(self, mask_list, pc_list, prob_list):
        ...
        if np.array(valid_mask).sum() > 0:
            ...
            # completion preprocessing (same as VAE version)
            ...
            # if completion output point count != 1024, resample to 1024
            B, N, _ = comp_output.shape
            if N != 1024:
                if N > 1024:
                    idx = torch.randperm(N, device=comp_output.device)[:1024]
            ...
            with torch.no_grad():
                while head < comp_output.shape[0]:
                    cur_chunk = comp_output[head:min(head + self.sdf_bs, comp_output.shape[0])]
                    pred_R_inv = self.objnet.VAE.get_pred_R_inv(cur_chunk)
                    cond_feat = self.objnet.cond_net(torch.bmm(cur_chunk, pred_R_inv).detach())
                    embedd = [self.ddim_sample(cond_feat), pred_R_inv]

                    canonical_query_pc = cur_chunk
                    mesh_pts, validness = self.extract_shape_pts(
                        self.objnet.VAE.decode, embedd, sample_pts_num=comp_output.shape[1], N=32
                    )
    ...
    def train_model(self, epochs):
        ...
        self.refresh_info()
        import time  # Add this import at the top of your file if not already present

        for epoch in range(start, epochs):
            for batch_idx, batch in enumerate(train_data_loader):
                # print(f'train batch {batch_idx+1} of {len(train_data_loader)}')
        ...
        for batch_idx, batch in enumerate(val_data_loader):
            ...
            if vis:
                with torch.no_grad():
                    ...
                    os.makedirs(self.cfg.save_path + '/vis_synthetic_ddpm/'+ area_name, exist_ok=True)
                    predcolor, gtcolor = np.ones_like(full_pc) * 128, np.ones_like(full_pc) * 128
                    for mask_id in range(valid_masks.shape[1]):
                        predcolor2 = np.ones_like(full_pc) * 128
                        mask = valid_masks[:, mask_id]
                        ...
                        predcolor[mask] = pred_instance_color[mask_id]
                        # write_ply(os.path.join(self.cfg.save_path + '/vis', scene_name[0] + 'preds_'+str(mask_id)+'.ply'), [full_pc, predcolor2.astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])
                    write_ply(os.path.join(self.cfg.save_path + '/vis_synthetic_ddpm/', area_name, room_name + 'preds.ply'), [full_pc[non_ceiling_mask], predcolor[non_ceiling_mask].astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])
                    ...
                    write_ply(os.path.join(self.cfg.save_path + '/vis_synthetic_ddpm/', area_name, room_name + 'gt.ply'), [full_pc[non_ceiling_mask], gtcolor[non_ceiling_mask].astype(np.uint8)], ['x', 'y', 'z', 'red', 'green', 'blue'])
